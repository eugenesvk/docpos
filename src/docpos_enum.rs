...
pub fn docpos_enum(mut enm:ItemEnum) -> proc_macro::TokenStream {
  ...
  let enum_docs = try2!(extract_struct_doc_attrs(&mut enm.attrs)); // extrac the doc attributes on the struct itself

  let (doc_params_to_enm, doc_fields) = try2!(extract_doc_variants_shift_up(enm.variants.iter_mut()));
  ...
  if !has_doc_fields {return syn::Error::new_spanned(enm.ident,"Enum has no documented variants or generics. Document at least one.",).into_compile_error().into();}

  let parameter_doc_block = make_doc_block("Variants", doc_fields);
